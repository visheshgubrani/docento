...
function resolveResumeLessonId(content?: StudentCourseContentSummaryResponse['data']) {
    const allLessons =
...
export async function getDashboardData() {
    try {
        ...
        try {
            const studentCoursesResponse = await fetchAPI<StudentCoursesResponse>('/student/courses', {
                requireAuth: true,
            })
            ...
            const resumeEntries = await Promise.all(
                studentCourses.map(async (course) => {
                    try {
                        const contentResponse = await fetchAPI<StudentCourseContentSummaryResponse>(
                            `/student/courses/${course.id}`,
                            { requireAuth: true }
                        )
                        return [course.id, resolveResumeLessonId(contentResponse.data)] as const
                    } catch {
        ...
        const enrollments = profile.profile.enrollments.map((enrollment) => ({
            ...
            progress: Math.max(0, Math.min(100, progressByCourseId.get(enrollment.course.id) ?? 0)),
            resumeLessonId: resumeLessonIdByCourseId.get(enrollment.course.id) ?? null,
            course: {
...
export async function getPurchaseHistoryData() {
    try {
        ...
        const ordersResponse = await fetchAPI<StudentOrdersResponse>('/student/orders', {
            requireAuth: true,
        })
        ...
        const paidOrderCourseIds = new Set(paidOrders.map((order) => order.course.id))
        ...
        const historyOrders = [...paidOrders, ...freeEnrollmentOrders].sort((a, b) => {
...
export async function getOrderReceipt(orderId: string) {
    try {
        ...
        const res = await fetchAPI<{ data?: OrderDocumentResponse }>(`/commerce/orders/${orderId}/receipt`, { requireAuth: true })
        if (!res.data) return { success: false as const, error: 'Failed to fetch receipt' }
...
export async function getOrderInvoice(orderId: string) {
    try {
        const profile = await getProfile()
        if (!profile) return { success: false as const, error: 'Not authenticated' }
        const res = await fetchAPI<{ data?: OrderDocumentResponse }>(`/commerce/orders/${orderId}/invoice`, { requireAuth: true })
        if (!res.data) return { success: false as const, error: 'Failed to fetch invoice' }
        return { success: true as const, data: res.data }
    } catch (error) {
