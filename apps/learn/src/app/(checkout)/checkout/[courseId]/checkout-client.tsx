...
async function postCommerce<T>(
  ...
  const payload = (await response
    .json()
    .catch(() => null)) as CommerceApiResponse<T> | null
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      const nextTotalAmount =
        ...
        Math.max(courseAmount - nextDiscountAmount, 0)
      ...
      if (!order?.orderId || !order?.keyId || !order?.amount) {
        setSuccessMessage(
          checkoutResponse?.message ||
            'Enrollment completed. Redirecting to your dashboard...',
        )
                ...
                <div className="mt-4 flex items-center gap-2">
                  <Input
                    ...
                    onChange={(event) => {
                      ...
                      if (
                        appliedCoupon &&
                        normalizedNext !== appliedCoupon.code
                      ) {
                        setAppliedCoupon(null)
                      }
                    }}
