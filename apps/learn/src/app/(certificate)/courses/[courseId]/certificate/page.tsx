...
import type { StorefrontCourseDetail, StorefrontCourseViewer } from "@/lib/lms-api-client";
  ...
  try {
    ...
    if (!courseTitle) {
      return {
        ...
        description: "Preview your Acme Learning course certificate.",
      };
    }
...
export default async function CourseCertificatePreview({
  ...
  try {
    const [profile, courseResponse, certificateResponse] = await Promise.all([
      ...
      fetchAPI<CertificateResponse>(`/student/courses/${courseId}/certificate`, {
        method: "POST",
        requireAuth: true,
        body: JSON.stringify({}),
      }),
    ...
    const instructorName = course.instructors?.[0]?.name?.trim() || "our instructor team";
    const lessonsCount = course.modules.reduce((total, module) => total + module.lessons.length, 0);
    const completionDateValue = certificate.issuedAt || viewer.enrollment?.completedAt || null;
    ...
    return (
      <section className="min-h-screen bg-background py-8 sm:py-10">
        <CourseCertificatePreviewPage
          ...
          totalCourseLength={course.includes?.videoDurationText || "Course completed"}
