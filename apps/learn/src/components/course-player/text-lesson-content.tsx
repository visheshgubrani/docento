...
export function TextLessonContent({
  ...
  /**
   * Both branches of `renderLessonHtml` sanitize.
   *
   * The TipTap path serializes an author-supplied JSON document; `serializeNode`
   * escapes text and attribute values, so it is not itself the hole — but it is
   * a second renderer, and a second renderer is a second thing that has to be
   * right. A serializer is also exactly the kind of code an editor extension
   * widens later (a new node type, a new mark attribute), and the widening would
   * not be visible from here. Routing its output through the same DOMPurify
   * allowlist as the legacy HTML path removes the question entirely.
   */
  const html = useMemo(() => renderLessonHtml(content), [content])
  ...
  if (html) return <article dangerouslySetInnerHTML={{ __html: html }} />

  if (plainText) return <p>{plainText}</p>
