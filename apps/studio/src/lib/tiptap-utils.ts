...
/**
 * Handles image upload with progress tracking and abort capability (PLACEHOLDER - for demo/testing)
...
/**
 * Creates an image upload handler for a specific lesson that uploads to Cloudflare R2
 * @param projectId The project ID
 * @param courseId The course ID
 * @param moduleId The module ID
 * @param lessonId The lesson ID
 * @returns An upload handler function for use with Tiptap ImageUploadNode
 */
export const createLessonImageUploadHandler = (
  projectId: string,
  courseId: string,
  moduleId: string,
  lessonId: string
  ...
  return async (
    ...
    try {
      ...
      const uploadSession = await createLessonImageUpload(
        projectId,
        courseId,
        moduleId,
        lessonId,
      ...
      const uploadResponse = await fetch(uploadSession.presignedUrl, {
