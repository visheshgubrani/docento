    ...
    <section ref={ref} id="features">
      <div className="container">
        <div className="max-w-6xl mx-auto">
          <div className="mx-auto my-12 h-full grid lg:grid-cols-2 gap-14 md:gap-30 items-center">
            <div
              className={`h-[390px] min-h-[210px] w-auto  ${
                ltr && 'lg:order-1'
              }`}
            >
              {data[currentIndex]?.image ? (
                <motion.img
                  ...
                  alt="feature"
                  className="aspect-auto mask-b-from-90% mask-r-from-90% md:scale-120 h-full w-full object-cover object-left-top p-1"
                ...
                {data.map((item, index) => (
                  ...
                  >
                    {linePosition === 'left' || linePosition === 'right' ? (
                      ...
                      >
                        <div
                          ...
                          } origin-top bg-accent/70 transition-all ease-linear `}
                          style={{
                            transitionDuration:
                              currentIndex === index
                                ? `${collapseDelay}ms`
                    ...
                    <div className="flex items-center relative">
                      <div className="item-box w-15 h-15 bg-white border-secondary border-2 rounded-full sm:mx-6 mx-2 shrink-0 flex items-center justify-center">
                        {item.icon}
                      </div>

                      <div>
                        <AccordionTrigger className="text-[1.07rem] font-medium font-allerta pl-0">
                          {item.title}
                        </AccordionTrigger>

                        <AccordionTrigger className="justify-start text-left leading-6 mt-2 text-[16px] font-ibm pl-0">
                          {item.content}
                        </AccordionTrigger>
                      </div>
                    </div>
                  ...
                  <div className="absolute bottom-0 left-0 right-auto top-0 h-0.5 w-full overflow-hidden rounded-lg bg-neutral-300/50 dark:bg-neutral-300/30">
                    <div
                      className={`absolute left-0 top-0 h-full ${
                        currentIndex === index ? 'w-full' : 'w-0'
                      } origin-top bg-accent transition-all ease-linear`}
                      style={{
                        transitionDuration:
