...
export default function ProjectWebhooksPage() {
  ...
  const handleSendTest = async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Unable to send test.'
      toast({
        title: 'Test failed',
        description: message,
        variant: 'destructive',
    ...
    <div className='space-y-10'>
      ...
      <div className='sticky top-0 z-10 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between -mx-4 md:-mx-6 xl:-mx-10 px-4 md:px-6 xl:px-10 pt-0 pb-4'>
        <div className='space-y-3'>
          <h2 className='text-3xl font-semibold font-literata tracking-wide'>Webhooks</h2>
      ...
      <div className='space-y-4'>
        <div className='flex items-center gap-3'>
          <h3 className='text-sm font-semibold text-foreground/80'>Endpoint URL</h3>
          ...
          <form className='space-y-6' onSubmit={handleSave}>
            <div className='space-y-3'>
              ...
              <div className='space-y-2'>
                <Label className='font-semibold' htmlFor='webhook-url'>Webhook URL</Label>
                <Input
                  id='webhook-url'
                  type='url'
      ...
      <div className='space-y-4'>
        <div className='flex items-center justify-between'>
          <h3 className='text-sm font-semibold text-foreground/80'>Signing Secret</h3>
        </div>
        <div className='rounded-sm border border-neutral-200 bg-background p-5'>
          <div className='space-y-4'>
            <p className='text-sm text-foreground/60'>
              Use this secret to verify the <code className='text-foreground/80 bg-neutral-100 px-2 py-0.5 rounded font-medium text-xs'>X-LMS-Signature</code> header on your backend.
            </p>
            <div className='space-y-2 mt-2'>
              <Label className='font-semibold' htmlFor='webhook-secret'>Secret Key</Label>
      ...
      {!isLoading && !webhook?.url && (
        <div className='w-full bg-background rounded-sm border border-neutral-200 py-16 px-6'>
          <div className='flex flex-col items-center justify-center text-center'>
            ...
            <h3 className='text-2xl font-semibold text-foreground mb-2'>No webhook configured</h3>
            <p className='text-base text-foreground/60 max-w-md mb-6'>
              Add a webhook endpoint URL above to start receiving real-time events
              when students enroll, complete lessons, and more.
            </p>
          </div>
        </div>
      )}
