...
import { AIGenerationButton, ModuleSkeletonLoader } from '../components/ai-generation-box'
...
export default function CourseCurriculumPage() {
    ...
    const [lastGenerationInput, setLastGenerationInput] = useState<GenerateCourseOutlineInput | null>(null)
    ...
    const handleGenerateOutline = useCallback(async (data: {
        description: string
        ...
        try {
            ...
            toast({
                ...
                description: err instanceof Error ? err.message : 'Failed to generate course outline. Please try again.',
                variant: 'destructive',
            })
        } finally {
    ...
    const handleRegenerate = useCallback(async () => {
        ...
        try {
            const outline = await generateCourseOutline(projectId, lastGenerationInput)
            setGeneratedOutline(outline)
            ...
            toast({
                ...
                description: err instanceof Error ? err.message : 'Failed to regenerate course outline. Please try again.',
                variant: 'destructive',
            })
        } finally {
            setIsGenerating(false)
    ...
    const handleConfirmCreate = useCallback(async () => {
        ...
        try {
            ...
            if (course?.modules && course.modules.length > 0) {
                await Promise.all(
                    course.modules.map((mod) => deleteModule(projectId, courseId, mod.id))
                )
            }
            ...
            for (const generatedModule of generatedOutline.modules) {
                ...
                for (const generatedLesson of generatedModule.lessons) {
                    await createLesson(projectId, courseId, createdModule.id, {
                        ...
                        contentType: toPrimaryLessonContentType(generatedLesson.contentType),
            ...
            await queryClient.invalidateQueries({ queryKey: ['project-course', projectId, courseId] })
            ...
            toast({
                ...
                description: err instanceof Error ? err.message : 'Failed to create modules and lessons. Please try again.',
                variant: 'destructive',
            })
        } finally {
            setIsCreating(false)
    ...
    if (!projectId || !courseId) {
        return (
            <div className='rounded-sm border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive'>
                Missing course information. Select a course from the courses list to continue.
            </div>
        )
    }
    ...
    if (isError) {
        return (
            <div className='rounded-sm border border-destructive/30 bg-destructive/5 p-6'>
                <h3 className='text-lg font-semibold text-foreground'>Unable to load course</h3>
        ...
        <div className='space-y-6'>
            ...
            {isLoading ? (
                ...
                <div className='space-y-6'>
                    ...
                    {!isGenerating && <ManualCourseBuilder modules={course?.modules ?? []} />}
                </div>
            )}
                    ...
                    <div className='flex items-stretch'>
                        ...
                        <div className='flex-1'>
                            ...
                            <div className='divide-y divide-neutral-200'>
                                {[1, 2, 3].slice(0, moduleIndex === 1 ? 3 : 2).map((lessonIndex) => (
                                    <div
                                        key={lessonIndex}
