...
export default function QuizEditorPage() {
  ...
  const [lastGenerationOptions, setLastGenerationOptions] =
    useState<GenerateQuizOptions | null>(null)
  ...
  useEffect(() => {
    if (previewStorage.isHydrated && previewStorage.hasPreview()) {
      ...
      if (
        ...
        if (storedData.quizDescription)
          setQuizDescription(storedData.quizDescription)
        setIsPreviewOpen(true)

        toast({
          title: 'Quiz Preview Restored',
          description:
            'Your previously generated quiz preview has been restored.',
        })
      }
    }
  ...
  const handleSaveQuizDetails = async () => {
    ...
    try {
      ...
      const lessonTitle =
        currentLesson?.title || currentModule?.title || 'Lesson'
      const quizData = {
        title:
          quizTitle.trim() ||
          `${lessonTitle} ${isMockTestLesson ? 'Mock Test' : 'Quiz'}`,
        description:
      ...
      const message =
        error instanceof Error ? error.message : 'Failed to save quiz details'
      toast({
        title: 'Error',
        description: message,
        variant: 'destructive',
        ...
        if (hasQuestionText && hasCorrectAnswer) {
          try {
            const questionData = {
              ...
              options:
                tempQuestion.options?.filter((o) => o?.trim()) || undefined,
            ...
            setQuestions((prev) =>
              prev.map((q) => (q.id === tempQuestion.id ? newQuestion : q)),
            )
          } catch (error) {
    ...
    try {
      ...
      if (result.questions && result.questions.length > 0) {
        ...
        toast({
          ...
          description:
            'The AI could not generate questions. Please try again with a different description.',
          variant: 'destructive',
        })
      }
    } catch (error) {
  ...
  const handleRegenerateQuiz = async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Failed to regenerate quiz'
      toast({
        title: 'Error',
        description: message,
        variant: 'destructive',
