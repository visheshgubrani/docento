...
import { captureClientException, captureEvent, resetAnalytics } from "@/lib/posthog";
    ...
    <div className="flex min-h-screen bg-dashboard-bg">
      ...
      {!isProjectRoute && (
        <aside className={cn(
          ...
          <AgencySidebar pathname={pathname} user={user} onSignOut={handleSignOut} />
        </aside>
      )}
      {isProjectRoute && (
        <aside className="hidden lg:fixed lg:inset-y-0 lg:z-50 lg:flex lg:flex-col lg:w-20">
          <AgencySidebar pathname={pathname} user={user} onSignOut={handleSignOut} collapsed={true} />
        </aside>
      )}
      {!isProjectRoute && (
        ...
        {!isProjectRoute && (
          <div className="sticky w-full justify-between bg-white top-0 z-40 flex h-14 items-center gap-3 border-b border-border/40 px-4 md:hidden">
            <div className="flex items-center gap-2">
              ...
              />
              <span className="font-comfortaa text-xl font-bold text-foreground">Docento</span>
            </div>
            <Button
              variant="ghost"
              size="icon"
...
function AgencySidebar({ pathname, onNavigate, user, onSignOut, collapsed = false }: AgencySidebarProps) {
  return (
            ...
            <TooltipTrigger asChild>
              <div className={cn(
                ...
                <Avatar className="h-10 w-10">
                  ...
                  <AvatarFallback className="text-foreground/50 text-lg font-semibold bg-dashboard-bg">{user?.name?.charAt(0)?.toUpperCase() || "U"}</AvatarFallback>
                </Avatar>
                <div className={cn(
                  ...
                  <span className="mt-0.5 text-xs text-muted-foreground/80 break-words">{user?.email || "email"}</span>
                </div>
              </div>
            </TooltipTrigger>
            <TooltipContent side="right" className={cn(
