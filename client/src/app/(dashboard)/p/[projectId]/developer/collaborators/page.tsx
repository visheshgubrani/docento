...
import { Loader2, MailPlus, ShieldCheck, Trash2, UserPlus2, Users } from 'lucide-react'
    ...
    <div className='space-y-10'>
      <div className='sticky top-0 z-10 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between -mx-4 md:-mx-6 xl:-mx-10 px-4 md:px-6 xl:px-10 pt-0 pb-4'>
        <div className='space-y-3'>
          <h2 className='text-3xl font-semibold font-literata tracking-wide'>Collaborators</h2>
        ...
        <div className='rounded-sm border border-neutral-200 bg-background px-4 py-3 text-sm text-foreground/75'>
          <p className='font-semibold text-foreground/90'>{project?.name ?? 'Current project'}</p>
      ...
      {error ? (
        <div className='rounded-sm border border-destructive/30 bg-destructive/5 px-4 py-3'>
          <p className='text-sm font-medium text-destructive'>Unable to load collaborators</p>
      ...
      <div className='space-y-4'>
        <div className='flex items-center gap-3'>
          <h3 className='text-sm font-semibold text-foreground/80'>Invite Collaborator</h3>
        ...
        <div className='rounded-sm border border-neutral-200 bg-background p-5'>
          <form onSubmit={handleInvite} className='space-y-4'>
            <p className='text-sm text-foreground/65'>
              Enter an email to send a collaborator invitation.
      ...
      <div className='space-y-4'>
        <div className='flex items-center gap-3'>
          <h3 className='text-sm font-semibold text-foreground/80'>Team Members</h3>
          <div className='flex items-center gap-1.5 text-foreground bg-accent-foreground/50 px-2 py-0.5 rounded-sm'>
          ...
          {isLoading ? (
                      ...
                      <TableCell>
                        <div className='flex items-center gap-3'>
                          <Avatar className='h-10 w-10'>
                            <AvatarImage src={row.image ?? ''} alt={row.displayName} />
                            <AvatarFallback>
                      ...
                      <TableCell className='text-right'>
                        {row.isOwner || !row.memberId ? (
                          ...
                          <Button
                            variant='ghost'
                            size='sm'
                            onClick={() => handleRemoveMember(row.memberId!, row.email)}
                            disabled={isRemoving && removingMemberId === row.memberId}
            ...
            <div className='w-full rounded-sm border border-neutral-200 bg-background py-10 px-6'>
              <div className='flex flex-col items-center justify-center text-center'>
                ...
                <h3 className='text-xl font-semibold text-foreground mb-1'>No collaborators yet</h3>
                <p className='text-sm text-foreground/60 max-w-md'>
                  Invite teammates above to help build courses and manage content.
                </p>
              </div>
            </div>
          )}
      ...
      <div className='space-y-4'>
        <div className='flex items-center gap-3'>
          <h3 className='text-sm font-semibold text-foreground/80'>Pending Invitations</h3>
          <div className='flex items-center gap-1.5 text-foreground bg-accent-foreground/50 px-2 py-0.5 rounded-sm'>
            <MailPlus className='size-4' />
            <span className='text-xs font-medium'>{invitations.length} PENDING</span>
          </div>
        </div>
        <div className='rounded-sm border border-neutral-200 bg-background p-5'>
          {isLoading ? (
                ...
                <TableBody>
                  {invitations.map((invitation) => (
                    <TableRow key={invitation.id}>
                      <TableCell className='text-sm text-foreground'>{invitation.email}</TableCell>
                      <TableCell>
                        <Badge
                          variant='secondary'
                      ...
                      <TableCell className='text-right'>
                        <Button
                          ...
                          disabled={isRevoking && revokingInvitationId === invitation.id}
                          className='h-9 gap-1.5 cursor-pointer text-destructive hover:text-destructive hover:bg-destructive/10'
                        >
            ...
            <div className='w-full rounded-sm border border-neutral-200 bg-background py-10 px-6'>
              <div className='flex flex-col items-center justify-center text-center'>
                ...
                <h3 className='text-xl font-semibold text-foreground mb-1'>No pending invites</h3>
                <p className='text-sm text-foreground/60 max-w-md'>
                  When you invite new collaborators, pending invitations will appear here.
                </p>
              </div>
            </div>
          )}
