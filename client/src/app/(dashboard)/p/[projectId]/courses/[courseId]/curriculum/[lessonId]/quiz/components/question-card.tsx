...
export function QuestionCard({
    ...
    const validateAndSaveLastQuestion = async (): Promise<boolean> => {
        ...
        if (lastQuestion.id.startsWith('temp-')) {
            ...
            const isComplete = lastQuestion.questionText.trim() !== '' && lastQuestion.correctAnswer !== ''
            ...
            if (isComplete) {
                ...
                setValidationError('Please complete the current question before adding a new one.')
                return false
            }
        }
        ...
        if (isNewQuestion) {
            ...
            if (!currentQuiz) {
                try {
                    ...
                    currentQuiz = await createQuiz(projectId, courseId, moduleId, lessonId, {
                        title,
                        passingScore: 70,
                    })
    ...
    const handleSaveQuestionWrapper = async (questionId: string, data: CreateQuestionInput) => {
        try {
            ...
            {questions.length > 0 && (
                ...
                >
                    <div className='flex items-center justify-center gap-2'>
                        ...
                        <p className='text-lg font-medium text-foreground'>Add a new question</p>
                    </div>
                </button>
            )}
        </div>
