...
export function QuizPreviewBox({
    ...
    const handleEdit = () => {
        router.push(`/p/${projectId}/courses/${courseId}/curriculum/${lessonId}/quiz`)
    }
            ...
            <div className='rounded-md border border-neutral-300 bg-white overflow-hidden'>
                ...
                <div className='flex items-center justify-between px-5 py-4 bg-neutral-50 border-b border-neutral-200'>
                    <div className='flex items-center gap-3'>
                        ...
                        <h4 className='font-semibold text-xl text-foreground'>{quizLabel}</h4>
                    </div>
                ...
                <div className='px-5 py-5 space-y-3'>
                    <div>
                        <h5 className='font-semibold text-lg text-foreground'>{quiz.title}</h5>
                        {quiz.description && (
                            <p className='text-sm text-foreground/70 mt-1'>{quiz.description}</p>
                        )}
                    </div>
                    ...
                    <div className='grid grid-cols-2 md:grid-cols-4 gap-8 pt-6'>
                        <div className='flex items-start justify-center gap-2'>
                            ...
                            <div className='flex flex-col'>
                                <span className='text-sm font-semibold fcont-noto text-foreground/80'>Questions</span>
                                <span className='text-lg font-allerta text-foreground/90 font-semibold mt-1.5'>{questionCount}</span>
                            </div>
                        </div>
                        <div className='flex items-start justify-center gap-2'>
                            <FaListCheck className='size-4.5 text-accent/80 flex-shrink-0 mt-0.5' />
                            <div className='flex flex-col'>
                                <span className='text-sm font-semibold fcont-noto text-foreground/80'>Passing Score</span>
                                <span className='text-lg font-allerta text-foreground/90 font-semibold mt-1.5'>{quiz.passingScore || 70}%</span>
                            </div>
                        </div>
                        <div className='flex items-start justify-center gap-2'>
                            <RiLoopRightLine className='size-4.5 text-accent/80 flex-shrink-0 mt-0.5' />
                            <div className='flex flex-col'>
                                <span className='text-sm font-semibold fcont-noto text-foreground/80'>Max Attempts</span>
                        ...
                        <div className='flex items-start justify-center gap-2'>
                            ...
                            <div className='flex flex-col'>
                                <span className='text-sm font-semibold fcont-noto text-foreground/80'>Time Limit</span>
                                <span className='text-lg font-allerta text-foreground/90 font-semibold mt-1.5'>
                ...
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription className='text-foreground/60 mt-3'>
                            Are you sure you want to delete this {quizLabel.toLowerCase()}? This will also delete all questions and cannot be undone.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                        <Button
